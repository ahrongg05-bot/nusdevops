...
test('should allow duplicate todo items', async ({ page }) => {
  ...
  await expect(todoItems).toHaveCount(2);
  await expect(todoItems.nth(0)).toContainText('Buy milk');
  await expect(todoItems.nth(1)).toContainText('Buy milk');
});

test('should not allow an empty todo item', async ({ page }) => {
  const todoInput = page.locator('.new-todo');
